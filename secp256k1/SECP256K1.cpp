...
void Secp256K1::GetHash160_8(int type,bool compressed,
  ...
  if(type == P2PKH || type == BECH32) {
    ...
    alignas(32) uint8_t pk[8][64];
    alignas(32) uint8_t sh[8][32];
    alignas(32) uint8_t rh[8][20];

    GetPublicKeyRaw(true,k0,(char*)pk[0]);
    GetPublicKeyRaw(true,k1,(char*)pk[1]);
    GetPublicKeyRaw(true,k2,(char*)pk[2]);
    GetPublicKeyRaw(true,k3,(char*)pk[3]);
    GetPublicKeyRaw(true,k4,(char*)pk[4]);
    GetPublicKeyRaw(true,k5,(char*)pk[5]);
...
void Secp256K1::GetHash160(int type,bool compressed,
  ...
  case BECH32:
  {
    ...
    if (!compressed) {
      ...
      alignas(32) uint8_t pk[4][64];
      alignas(32) uint8_t sh[4][32];
      alignas(32) uint8_t rh[4][20];
      ...
      sha256_batch_33((uint8_t*)pk,4,(uint8_t*)sh);
      ripemd160_batch_32((uint8_t*)sh,4,(uint8_t*)rh);
